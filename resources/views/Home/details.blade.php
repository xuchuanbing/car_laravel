...
@endif
        ...
        <div class="det-picside" id="did">
            <img src='http://oslhf8hmf.bkt.clouddn.com/{{ $lists->picname }}' alt="{{ $lists->title }}">
        </div>
            ...
            <div class="dt-thumb-in" style="width:4000px;" data-role="thumbContainer">
                <ul class="dt-thumb-img">                
                    ...
                    <li id="aq" data-role="thumb" data-index="7"><a href="#">
                        <span></span>
                            <img class="{{ $v }}" style="height:61px" src='http://oslhf8hmf.bkt.clouddn.com/{{ $v }}'>
                        </a>
                    </li>
                    @endforeach
...
$(".dt-thumb-img li img").mouseover(function(){
    ...
    <div class="det-sumright appoint">
        <div class="dt-titbox">
            <h1 class="dt-titletype" title="{{ $lists->title }}">{{ $lists->title }}</h1>
            <div class="dt-titleinfo clearfix">
                ...
                <span style="color:#999">车源号：</span><span style="color:#000">{{ $lists->car_id }}</span>
            </div>
...
<!--            </div>-->
            ...
            <div class="car-fuwu">
                <i class="ico-txt">服&nbsp;务&nbsp;费：</i>
                                <span>{{ $lists->save_price }}（车价x4%，最低3500元）</span>
            </div>

            <ul class="assort clearfix">
                <li class="one"><b>{{ $list->licensing }}</b>上牌时间</li>
                <li><b>{{ $list->mileage }}</b>里程</li>
                <li><b>{{ $list->transmission }}</b>变速箱</li>
                <li class="em-sta detailHoverTips">
                    <em class="tip-icon"></em>
                    <p class="sta-tip-txt"><i></i>各地车管所对排放标准认定有差异，结果仅供参考，外迁请详细咨询迁入地车管所。</p>
                    <b>{{ $list->emission }}</b>排放标准</li>
                <li>
                    <b>{{ $list->address }}</b>上牌地
                </li>
            </ul>
                <p class="stipul-p">
    ...
    <div class="detect-bd clearfix">
            ...
            <div class="detect-c scrollTo">
                ...
                <div class="detect-info">
                    <div class="detect-txt">{{ $lists->introduce }}</div>
...
<div class="modbox" id="base">
        ...
        <ul class="owner-infor clearfix">
            <li class="owner">
                <label>车主：</label>
                @if ( $lists->source == 0) 官方自营 @else 第三方 @endif
                <i class="jg-line">|</i>
        ...
        <p class="f-type03"><em></em>{{ $lists->infomation }}</p>
    </div>
...
@foreach($car as $v)
            <img width="580" height="400" class="js-lazy-load" src='http://oslhf8hmf.bkt.clouddn.com/{{ $v }}'
...
@endforeach
        ...
        <div class="detecttitle">
            ...
            <span>检测时间：{{ $list->licensing }}</span>
                                        <span>检测车型：{{ $lists->title }}</span>
        </div>
        <div class="linetype"></div>
        <div class="headline-bd" id="accident">
            <span class="headline yahei">事故排查</span>
            <span class="f14">{{ $lists->introduce }}</span>
        </div><br>
...
<!--                <div class="bitpic"><img src="--><!--"></div>-->
        ...
        <div class="con-param">
            <div class="con-param-s clearfix">
                        ...
                        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="param-table">
                            ...
                            <tr><td width='50%' class='td1'>厂商</td><td width='50%' class='td2'>{{ $li->changshang }}</td></tr><tr><td width='50%' class='td1'>级别</td><td width='50%' class='td2'>{{ $li->jibie }}</td></tr><tr><td width='50%' class='td1'>发动机</td><td width='50%' class='td2'>{{ $li->fadongji }}</td></tr><tr><td width='50%' class='td1'>变速箱</td><td width='50%' class='td2'>{{ $li->biansuxiang }}</td></tr><tr><td width='50%' class='td1'>车身结构</td><td width='50%' class='td2'>{{ $li->cheshenjiegou }}</td></tr><tr><td width='50%' class='td1'>长*宽*高(mm)</td><td width='50%' class='td2'>{{ $li->c_k_g }}</td></tr><tr><td width='50%' class='td1'>轴距(mm)</td><td width='50%' class='td2'>{{ $li->zhouju }}</td></tr><tr><td width='50%' class='td1'>行李箱容积(L)</td><td width='50%' class='td2'>{{ $li->xl_rongji }}</td></tr><tr><td width='50%' class='td1'>整备质量(kg)</td><td width='50%' class='td2'>{{ $li->zhengbeizl }}</td></tr>                        </table>
                    </div>
                                    <div class="c-param-in">
                        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="param-table">
                            <tr>
                            ...
                            <tr><td width='50%' class='td1'>排量(L)</td><td width='50%' class='td2'>{{ $li->pailiang }}</td></tr><tr><td width='50%' class='td1'>进气形式</td><td width='50%' class='td2'>{{ $li->jingqixingshi }}</td></tr><tr><td width='50%' class='td1'>气缸</td><td width='50%' class='td2'>{{ $li->qigang }}</td></tr><tr><td width='50%' class='td1'>最大马力(Ps)</td><td width='50%' class='td2'>{{ $li->zuidamali }}</td></tr><tr><td width='50%' class='td1'>最大扭矩(N*m)</td><td width='50%' class='td2'>{{ $li->zuidazhouju }}</td></tr><tr><td width='50%' class='td1'>燃料类型</td><td width='50%' class='td2'>{{ $li->ranglianleixing }}</td></tr><tr><td width='50%' class='td1'>燃油标号</td><td width='50%' class='td2'>{{ $li->rangyoubiaohao }}</td></tr><tr><td width='50%' class='td1'>供油方式</td><td width='50%' class='td2'>{{ $li->gongyoufangshi }}</td></tr><tr title="各地车管所对排放标准认定有差异，结果仅供参考，外迁请详细咨询迁入地车管所。"><td width='50%' class='td1'>排放标准</td><td width='50%' class='td2'>{{ $li->paifangbiaozhun }}</td></tr>                        </table>
                    </div>
                                    <div class="c-param-in">
                        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="param-table">
                            <tr>
    ...
    <div class="guesslike">
        <ul class="list-bigimg clearfix">
                ...
                @foreach($lists_1 as $k=>$v)
        <li>
            <div class="list-infoBox">
                <a data-gzlog="tracking_type=click&eventid=0190140000000081&carid=13221362@3" title="{{ $v->title }}" gjalog="100000000082000100000010" target="_blank" class="imgtype"  href="{{ url('/home/details') }}/{{ $v->id }}" baidu_alog="pc_detail_tuijian&click&pc_detail_tuijian_detailCar_c">
                    <span class="hover-bg"></span>
                        <img width="290" height="192" src='http://oslhf8hmf.bkt.clouddn.com/{{ $v->picname }}'>
                            
                </a>
    <p class="infoBox">
        <a title="{{ $v->title }}" gjalog="100000000082000100000010" href="{{ url('/home/details') }}/{{ $v->id }}" target="_blank" class="info-title">
                                {{ $v->title }}                            </a>
    </p>
    <p class="fc-gray"><span class="">{{ $list_1[$k]->licensing }}上牌
    </span>
    <em class="shuxian">|</em>行驶{{ $list_1[$k]->mileage }}<em class="shuxian"></em><span class="tag-gray">{{ $list_1[$k]->address }}</span>                        
    </p>
    <p class="priType-s">
     <span><i class="fc-org priType">{{ $list_1[$k]->price }}</i>万</span>
        <s>16.26万</s>
    </p>
            </div>
        </li>
