...
<div class="person-main">
    ...
    <ul class="user-menu">
        ...
        <li><a data-gzlog="tracking_type=click&amp;eventid=1016123400000003" href="{{url('home/person')}}">降价提醒</a></li>
        <li><a data-gzlog="tracking_type=click&amp;eventid=1016123400000006" class="active" href="{{url('home/person/userhistory')}}">浏览记录</a></li>
    </ul>
</div>    <!-- 左菜单 e -->
         ...
         @foreach ($buyers as $K=>$v)
            <li class="list-infoBox js-oneInfo" puid="3001961610" clueid="14549078">
                <a href="https://www.guazi.com/bj/61798df0023f9ce0x.htm" class="info-img">
		          <img src='http://oslhf8hmf.bkt.clouddn.com/{{ $motor_details[$K]->picname }}'>
                </a>
		  <a href="{{url('home/userhistory/del')}}/{{ $v->uid }}" class="icon-delete js-delete-store-click">删除</a>
		  <a class="info-name" href="https://www.guazi.com/bj/61798df0023f9ce0x.htm">{{ $motor_details[$K]->title }}</a><p class="info-mile">{{ $detection_mes[$K]->licensing }}上牌<em>|</em>行驶{{ $detection_mes[$K]->mileage }}</p>
		  <p class="info-price"><span class="price-curr">{{ $detection_mes[$K]->price }}</span><span class="price-through">{{ $detection[$K]->price }}</span></p></li>
		 @endforeach
        </ul>
